...
void can1_init()
{
	...
	PINSEL1&=~(1<<19); //CAN Pin Selection
	PINSEL1=(1<<18);
...
void can1_tx(CAN_MSG m1)
{
	...
	{
		C1TFI1&=~(1<<30); //Configuring RTR bit RTR=0
		C1TDA1=m1.byteA;  // Data A Register for Transmit 
		C1TDB1=m1.byteB;  // Data B Register for Transmit
	}
	else	
		C1TFI1|=(1<<30);
	
	C1CMR=(1<<0)|(1<<5);     //Start Xmission & use Tx Buf1
	while((C1SR&(1<<3))==0);  //Wait for transmission to complete		
}
